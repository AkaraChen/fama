...
extern "C" {
    fn FormatShell(source: *const c_char, source_len: size_t, indent: c_uint) -> *mut c_char;
    fn FormatShellBatch(
...
pub fn format_shell(source: &str, _file_path: &str) -> Result<String, String> {
    let c_source = CString::new(source).map_err(|e| format!("Invalid source: {}", e))?;
    let c_result = unsafe { FormatShell(c_source.as_ptr(), source.len() as size_t, get_indent()) };
...
pub fn format_shell_batch(sources: &[&str]) -> Vec<Result<String, String>> {
    ...
    let c_sources: Vec<CString> = match sources.iter().map(|s| CString::new(*s)).collect() {
        Ok(v) => v,
        Err(_) => return sources.iter().map(|_| Err("Invalid source".to_string())).collect(),
    };
    ...
    let c_results = unsafe {
        FormatShellBatch(c_ptrs.as_ptr(), lengths.as_ptr(), sources.len() as size_t, get_indent())
    };

    if c_results.is_null() {
        return sources.iter().map(|_| Err("Formatter returned null".to_string())).collect();
    }
